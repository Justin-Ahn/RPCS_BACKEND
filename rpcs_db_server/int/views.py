...
from int.models import CaregiverProfile, DoctorProfile, PatientProfile, Patient_incidents
...
def patient_incidents(request):
    ...
    if request.method == "GET":
        return return_data(request, Patient_incidents, filterable_params)
    elif request.method == "POST":
        return ingest_data(request, Patient_incidents, my_fields, json_customizer=json_timestamp_customizer)
    else:
        return handle_invalid_request(request)
